...
<el-card v-if="allowed" shadow="never" style="margin-bottom:20px"><el-form inline @submit.prevent="upload"><el-form-item label="版本"><el-input v-model="form.version" placeholder="例如 8.0.36" /></el-form-item><el-form-item label="架构"><el-select v-model="form.architecture" style="width:130px"><el-option label="AMD64" value="amd64"/><el-option label="ARM64" value="arm64"/></el-select></el-form-item><el-form-item label="tar.gz 文件"><input type="file" accept=".gz,.tgz" @change="file=($event.target as HTMLInputElement).files?.[0]" /></el-form-item><el-button native-type="submit" type="primary" :loading="busy">上传</el-button></el-form><el-progress v-if="busy" :percentage="progress" /></el-card>
