...
- 复制基线已完成一次真实 XtraBackup 生成、传输、prepare 和恢复验收；平台复制创建流程仍要求 `baseline_ready=true`，物理备份任务已接入 Agent executor，原主机物理恢复完成副本校验、prepare 和暂存 copy-back，最终切换保留人工核查；新主机逻辑/物理恢复已支持自动传输、创建并启动全新实例，无需确认。
...
- 归档已完成隔离小表的真实数据一致性、负载保护和暂停续跑验收；生产大表性能、跨目标实例一致性、持久游标和长期运行故障仍需专门验证。
...
新增跨主机自动恢复验收：任务 47（mysqldump）和 48（XtraBackup）从 clp01 恢复到 clp02 的新实例 13312/13313，六条记录一致、新 UUID/server_id、无旧复制通道，任务 49/50 重启成功。请求未传 confirmed；原主机无确认请求及借新主机入口恢复到原主机均被拒绝。
