...
暂停作业可调用 `/mysql/archive/jobs/:id/resume` 续跑；运行中保护触发的 failed 作业需先核对源表、目标表和副本，再创建新作业处理剩余行。归档跨批次可能已有提交，任务 failed 不表示数据回滚；仅在 Agent 返回可解析统计时，作业计数才包含已处理行。无法取得统计时以实际表数据为准，禁止直接假定计数 0 意味着未写入。生产大表上线前还需验证索引选择、保留条件、负载和目标空间。
