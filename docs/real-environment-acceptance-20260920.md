...
### pt-archiver 真实归档与保护验收

clp01 安装 Percona Toolkit 3.2.1 的 `pt-archiver`；仅对新建的 `archive_fixture.events` / `events_archive` 测试表操作，clp02 通过现有 GTID 复制接收变更。归档策略指定主键范围、同实例目标、每批 5 或 20 行，并启用源表删除。首次任务 51 发现本机工具拒绝小数形式的 `--sleep 0.010`，修复为正毫秒向上取整到整数秒。任务 52 实际归档 100 行；随后修复真实工具输出 `SELECT/INSERT/DELETE N` 的统计解析，任务 53 再归档 20 行，作业计数为扫描/归档/删除各 20 行，主从两表行数一致。失败任务保留原状态，没有改写为成功。

| 场景 | 任务与观察 | 结果 |
|---|---|---|
| 源库负载超限 | 并发 `SELECT SLEEP(8)` 期间 `Threads_running=4`，策略上限 1；任务 54 报错，目标范围的源表 5 行仍在、目标 0 行 | 通过 |
| 启动前复制故障 | 停止 clp02 SQL 线程，策略最大延迟 1 秒；任务 55 被副本健康检查拒绝，目标范围源表 5 行仍在、目标 0 行；线程恢复后 healthy | 通过 |
| 暂停续跑 | 任务 56 在 40 行中归档 20 行后暂停，任务 58 续跑后源表 0、归档表 40，主从一致且 ID 唯一；累计扫描 45 行含暂停边界的回滚批次 | 通过 |
| 运行中复制故障 | 任务 59/61 运行时停止 clp02 SQL 线程，Server 主动下发 `mysql.archive.stop`；任务均 failed，复制线程已恢复且追平 | 通过 |
| 部分提交及再处理 | 任务 61 已提交 30 行、源表剩余 10 行，作业记录扫描 35、归档/删除各 30；源/目标合计 40 行，两台主机一致。人工核查后新建任务 62 处理剩余 10 行，最终归档 40 个唯一 ID、源表 0 行 | 通过 |

任务 59 最初暴露失败作业计数为 0、但数据库已有 30 行提交的问题；修复后任务 61 保留 Agent 返回的部分统计。任务 60 因 Server 重启后 clp02 Agent 短暂离线，在前置保护阶段失败，未操作归档范围；Agent 重连后再执行。运行中保护是周期检查，故障触发前已提交的批次不会回滚，故失败作业仍需按运维流程核对源/归档表。此验收未覆盖生产大表吞吐、跨实例目标或长期游标恢复。
